...
const LAYOUTS = {
  "1080p": {
    ...
    slots: {
      ...
      amulet:   { x: 1240, y: 120 },
      vest:     { x: 1240, y: 280 },
      trinket1: { x: 1240, y: 440 },
      trinket2: { x: 1240, y: 600 },
      pants:    { x: 1240, y: 760 },
    }
  },
  ...
  "1440p": {
    ...
    slots: {
      ...
      amulet:   { x: 1620, y: 120 },
      vest:     { x: 1620, y: 280 },
      trinket1: { x: 1620, y: 440 },
      trinket2: { x: 1620, y: 600 },
      pants:    { x: 1620, y: 760 },
    }
  }
};
...
function getActiveLayout(){
  ...
  // Most 1920x1080 players should use the 1080p-tuned layout.
  // 2560x1440 and above use your working 1440p-tuned layout.
  if(w <= 2100 || h <= 1200){
    return LAYOUTS["1080p"];
  }

  return LAYOUTS["1440p"];
}
...
function positionFloatingSlots(){
  ...
  wrappers.each(function(){
    ...
    const fixedPos = layout.slots[slot];
    if(!fixedPos) return;

    const pos = STATE.bonePositions && STATE.bonePositions[slot];
    const boneX = pos ? (pos.x * layout.frameW) : (layout.frameW / 2);
    const boneY = pos ? (pos.y * layout.frameH) : (layout.frameH / 2);
...
function drawBoneLines(slotEntries){
  ...
  for(const entry of slotEntries){
    ...
    // pos.x/pos.y are normalized screen coordinates from RedM.
    // Multiplying by 1920/1080 keeps this in the same scaled UI-frame design space.
    const layout = getActiveLayout();

	const targetX = pos.x * layout.frameW;
	const targetY = pos.y * layout.frameH;
    ...
    const startX = slotCenterX + (dx / dist) * halfSlot;
    const startY = slotCenterY + (dy / dist) * halfSlot;

    // Smooth curve without the old wave offset that made long lines go wild.
    const curveStrength = Math.min(Math.abs(dx) * 0.22, 70);
    const dir = targetX >= slotCenterX ? 1 : -1;

    const cp1x = startX + curveStrength * dir;
    const cp1y = startY;
    const cp2x = targetX - curveStrength * dir;
    const cp2y = targetY;

    const path = document.createElementNS(ns, "path");
    path.setAttribute("d", `M ${startX} ${startY} C ${cp1x} ${cp1y}, ${cp2x} ${cp2y}, ${targetX} ${targetY}`);
    path.setAttribute("fill", "none");
    path.setAttribute("stroke", "rgba(155, 20, 18, 0.88)");
    ...
    dot.setAttribute("fill", "rgba(155, 20, 18, 0.95)");
    dot.setAttribute("filter", "url(#bone-line-glow)");
...
function setHudHidden(hidden, save){
  STATE.hudHidden = !!hidden;

  if(save){
    try {
      window.localStorage.setItem("cas_armour_hud_hidden", STATE.hudHidden ? "1" : "0");
    } catch(e) {}
  }

  if(STATE.ui.$hud && STATE.ui.$hud.length){
    STATE.ui.$hud.toggleClass("cas-armour-hud-hidden", STATE.hudHidden);
    if(STATE.hudHidden){
      STATE.ui.$hud.removeClass("is-active");
    }
  }
...
function toggleHudHidden(){
  setHudHidden(!STATE.hudHidden, true);
}
...
function renderStats(){
  ...
  window.addEventListener("message", function(event){
    ...
    if(data.action === "cas_armour:setHudHidden"){
      setHudHidden(!!data.hidden, true);
      return;
    }
    if(data.action === "cas_armour:toggleHudHidden"){
      toggleHudHidden();
      return;
    }
  ...
  $(document).ready(function(){
    ...
    try {
      STATE.hudHidden = window.localStorage.getItem("cas_armour_hud_hidden") === "1";
      if(STATE.ui.$hud && STATE.ui.$hud.length){
        STATE.ui.$hud.toggleClass("cas-armour-hud-hidden", STATE.hudHidden);
      }
    } catch(e) {}
