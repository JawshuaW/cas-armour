...
local function SendBonePositionsToNui()
  CreateThread(function()
    -- Let the cinematic camera settle, then refresh a few times.
    -- This keeps connector lines lined up without chasing idle movement forever.
    Wait(350)

    for i = 1, 10 do
      if not ClientState.isUiOpen then break end
    ...
    dprint('Bone positions sent/refreshed to NUI')
  end)
end
...
RegisterNetEvent('cas-armour:client:openMenu', function()
  OpenArmourMenu()
end)
...
local armourHudHidden = false

local function SetArmourHudHidden(hidden)
  armourHudHidden = hidden and true or false

  SendNUIMessage({
    action = 'cas_armour:setHudHidden',
    hidden = armourHudHidden
  })

  local msg = armourHudHidden and 'Armor HUD hidden. Use /showarmor to show it again.' or 'Armor HUD shown.'
  TriggerEvent('vorp:TipRight', msg, 3500)
end

RegisterCommand('hidearmor', function()
  SetArmourHudHidden(true)
end, false)

RegisterCommand('hidearmour', function()
  SetArmourHudHidden(true)
end, false)

RegisterCommand('showarmor', function()
  SetArmourHudHidden(false)
end, false)

RegisterCommand('showarmour', function()
  SetArmourHudHidden(false)
end, false)

RegisterCommand('togglearmorhud', function()
  SetArmourHudHidden(not armourHudHidden)
end, false)

RegisterCommand('togglearmourhud', function()
  SetArmourHudHidden(not armourHudHidden)
end, false)
